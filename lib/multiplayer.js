...
// Récupère la liste des joueurs d'une salle (id, identité, statut, mute) —
// utilisé par les 3 pages de jeu pour afficher le chat (résoudre pseudo/
// avatar par player_id) et donner à l'hôte la liste des joueurs qu'il peut
// modérer. Facteur commun de ce que faisait déjà la page salle d'attente.
export async function fetchRoomPlayers(supabase, gameId) {
  const { data, error } = await supabase
    .from('multiplayer_players')
    .select('id, user_id, pseudo, is_guest, avatar_url, status, muted, joined_at, role, is_premium')
    .eq('game_id', gameId)
    .order('joined_at', { ascending: true })
  return error ? [] : (data || [])
}

// Abonnement temps réel dédié au chat d'une salle — canal séparé de
// subscribeRoomRealtime plutôt que d'alourdir/risquer de perturber le canal
// déjà en place pour la synchronisation critique (avancement de question,
// changement de phase...) : le chat est une fonctionnalité annexe, autant
// qu'elle vive sur son propre canal indépendant.
export function subscribeChatRealtime(supabase, { gameId, onMessage }) {
  const channel = supabase.channel(`multiplayer_chat_${gameId}`)
  channel
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'multiplayer_chat_messages', filter: `game_id=eq.${gameId}` }, (payload) => {
